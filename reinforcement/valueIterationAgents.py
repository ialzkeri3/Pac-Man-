...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount=0.9, iterations=100):
        ...
        for i in range(iterations):
            estimatetdValues = self.values.copy()

            for state in self.mdp.getStates():
                if not self.mdp.isTerminal(state):

                    # Finding best value for possible actions
                    #reward = self.mdp.getReward(state)
                    actions = self.mdp.getPossibleActions(state)
                    value = max([self.computeQValueFromValues(state, a)
                                 for a in actions])
                    estimatetdValues[state] = value

            self.values = estimatetdValues
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for nextState, probability in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            qValue += probability * \
                (nextReward + self.discount * self.values[nextState])
