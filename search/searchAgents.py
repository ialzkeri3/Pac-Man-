    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if (nextx, nexty) in cornersToVisit:
                    cornersToVisit.remove((nextx, nexty))
                cornersToVisit = tuple(cornersToVisit)
                successors.append(
                    (((nextx, nexty), cornersToVisit), action, 1))
        self._expanded += 1
        return successors
...
def cornersHeuristic(state, problem):
    ...
    for i in state[1]:
        currentDistance = util.manhattanDistance(state[0], i)
...
class AStarCornersAgent(SearchAgent):
    "A SearchAgent for FoodSearchProblem using A* and yours foodHeuristic"
...
def foodHeuristic(state, problem):
    ...
    for food in foodGrid.asList():
        currentDistance = util.manhattanDistance(position, food)
        if currentDistance > maxDistance:
            maxDistance = currentDistance
