...
class ExactInference(InferenceModule):
        ...
        if not noisyDistance == None:
            for position in self.legalPositions:
                trueDist = util.manhattanDistance(
                    position, pacmanPosition)     # true distance
                allPossible[position] = self.beliefs[position] * \
                    emissionModel[trueDist]
        else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldGhosts in self.beliefs:
            self.setGhostPosition(gameState, oldGhosts)
            newDist = self.getPositionDistribution(
                gameState)   # new distibution
            for p, probability in newDist.items():
                newBeliefs[p] += probability * self.beliefs[oldGhosts]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        N = self.numParticles       # number of particles
    ...
    def observe(self, observation, gameState):
        ...
        for position in self.particles:
            if noisyDistance == None:
                tempC[self.getJailPosition()] = 1.0
            else:
                trueDist = util.manhattanDistance(
                    position, pacmanPosition)
                tempC[position] = oldBelief[position] * \
                    emissionModel[trueDist]
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            newDist = self.getPositionDistribution(         # Distribution
                self.setGhostPosition(gameState, particle))
            updatedParticles.append(util.sample(newDist))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        posList = list(itertools.product(
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            index = 0
            while index < self.numGhosts:
                newParticle[index] = util.sample(getPositionDistributionForGhost(
                    gameState, index, self.ghostAgents[index]))
                index = index + 1
