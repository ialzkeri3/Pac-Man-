...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        bestAction = 'Stop'  # arbitrary action
    ...
    def update(self, state, action, nextState, reward):
        ...
        rewardPlusNextQ = reward

        if nextState:
            rewardPlusNextQ += self.discount * \
                self.computeValueFromQValues(nextState)

        updatedValue = self.alpha * rewardPlusNextQ

        updatedValue += (1-self.alpha) * self.getQValue(state, action)
